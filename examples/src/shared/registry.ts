...
import LoanModelFiles474       from '../demos/474-loan-model-files.svelte'
import SheetChartsObjects475   from '../demos/475-sheet-charts-objects.svelte'
import SheetSparklines476      from '../demos/476-sheet-sparklines.svelte'
import SheetPivotRange477      from '../demos/477-sheet-pivot-range.svelte'
import SheetCollaboration478   from '../demos/478-sheet-collaboration.svelte'
import SheetHyperlinks479      from '../demos/479-sheet-hyperlinks.svelte'
import SheetLetLambda480       from '../demos/480-sheet-let-lambda.svelte'
import SheetTables481          from '../demos/481-sheet-tables.svelte'
import SheetIterative482      from '../demos/482-sheet-iterative.svelte'
import SheetAuditing483       from '../demos/483-sheet-auditing.svelte'
import SheetCellImages484     from '../demos/484-sheet-cell-images.svelte'
...
export const demos: Demo[] = [
  ...
  demo('474-loan-model-files', 'Loan model: PMT, accounting formats and the File tab', 'A mortgage model built from the sheet\'s own financial functions: PMT for the payment, IPMT and PPMT for each period\'s split, NPER and RATE for the term and rate that fit, SUMPRODUCT for the first year of interest. The money cells wear the Accounting format, Extra Payment carries a validation rule with an Input Message, and Circle Invalid Data rings what breaks it. The File tab is the point: Save As writes the whole model as an .xlsx Excel opens with its formulas, formats and rules, Save As ODS writes the same model for LibreOffice Calc and Save As XLS the Excel 97-2003 binary, Open reads any of them (or a .csv) back by their bytes rather than their name, and Export CSV takes the active sheet.', 'Spreadsheet', LoanModelFiles474, { pro: true }),
  demo('475-sheet-charts-objects', 'Charts and pictures on the sheet', 'Charts anchored over the cells the way Excel anchors one: each reads a range rather than a copy of the numbers, so typing into a cell redraws it. Insert > Chart charts the selected block and reads its first row and column as the labels, Insert > Picture puts an image on the sheet, and a double-click opens the Chart dialog for the type, the title, series in columns or rows and stacking. Drag an object to move it, its corner to resize, Delete to remove. It hangs from a cell, so inserting a row above moves it, and it rides in getState() and in the .xlsx as a real drawing part.', 'Spreadsheet', SheetChartsObjects475, { pro: true }),
  demo('476-sheet-sparklines', 'Sparklines: a chart inside the cell', 'Excel\'s smallest chart, and not an object: a sparkline IS the cell. One per row of a block of numbers, drawn from the range rather than a copy, so editing a number redraws it. Insert > Sparklines offers Line, Column and Win/Loss, with Edit for the group\'s ranges, kind and colours and Clear for the groups the selection touches; selecting a cell that holds one turns the kind buttons into a change to that group. They are kept per group the way Excel keeps them, ride in getState() and in the .xlsx where Excel keeps them, and move with an insert or a delete.', 'Spreadsheet', SheetSparklines476, { pro: true }),
  demo('477-sheet-pivot-range', 'PivotTable from a range', 'Excel\'s Insert > PivotTable over a block of cells, on the same pivot engine the grid uses for its own pivot mode. The sheet keeps the definition - the source block, where the result goes, and which field is a row, a column or a measure - and writes the result as plain cells in one undo, so it can be formatted, charted, printed and saved to an .xlsx like any other block. Show Details writes the source rows behind a cell to a sheet of their own, and Refresh rebuilds it from the source; opening the dialog from inside one edits it.', 'Spreadsheet', SheetPivotRange477, { pro: true }),
  demo('478-sheet-collaboration', 'Two people on one sheet: the delta stream', 'Two full spreadsheets over two separate documents, wired to each other by createDeltaStream: type in either and the other follows. What crosses the wire is a delta rather than the document, and the log shows each one as it goes - a formula travels as its text so the other side works out its own answer, an insert travels as the edit so both rewrite their own formulas, a format travels as the one part of the one sheet that changed. Presence rides the same wire and is never part of the document: each window draws the other\'s selection as a coloured box with a name on it. Conflicts are last writer wins, per cell. A real app puts a socket in the middle.', 'Spreadsheet', SheetCollaboration478, { pro: true }),
  demo('479-sheet-hyperlinks', 'Hyperlinks: Insert > Link and HYPERLINK', 'Excel\'s two ways of putting a link in a cell. Insert > Link (Ctrl+K) puts one ON the cell, so editing the text keeps it and clearing the cell takes it away; the HYPERLINK function puts one in a formula with a friendly name. A target that reads like an address moves the selection instead of leaving the page, so a cell links to another sheet; anything with a scheme opens in a new tab. A single click follows, a drag selects. Links ride in getState(), move with an insert, and go into the .xlsx both ways.', 'Spreadsheet', SheetHyperlinks479, { pro: true }),
  demo('480-sheet-let-lambda', 'LET and LAMBDA', 'The two modern Excel functions that turn a formula into something you can read, and the helpers that make a lambda worth writing. LET names a value inside the formula so it is written once and read by name; LAMBDA is a function written in the sheet, bound by LET and called by name, or called where it stands. MAP, BYROW, BYCOL, REDUCE, SCAN and MAKEARRAY put one over every cell, every row, every column, a fold, a running total, and an array built from its own indexes. They spill, and they go into the .xlsx under the prefix Excel stores them with.', 'Spreadsheet', SheetLetLambda480, { pro: true }),
  demo('481-sheet-tables', 'Format as Table and structured references', 'Excel\'s Ctrl+T: a block becomes a table with a header row, the banded look, filter arrows and a name its columns are read by, so a formula says Orders[Amount] instead of E2:E13 and keeps meaning it as rows are added. [@Qty] is this row\'s cell. Auto-expand is the point: typing under the last row grows the table, and every total that reads it grows too, with no formula re-pointed. Insert > Table Styles picks the look from eighteen presets named the way Excel names them. Tables ride in getState() and go into the .xlsx as real table parts, style included.', 'Spreadsheet', SheetTables481, { pro: true }),
  demo('482-sheet-iterative', 'Iterative calculation', 'A circular reference is normally an error, and every cell in the loop shows #CYCLE!. Some models are circular on purpose: a bonus that is a share of the profit it is taken out of, interest charged on the balance it is part of. Formulas > Calculation Options turns on iterative calculation, and the loop runs until it stops moving or the passes run out. The setting rides in getState() and goes into the .xlsx as calcPr, so the file opens the same way in Excel.', 'Spreadsheet', SheetIterative482, { pro: true }),
  demo('483-sheet-auditing', 'Evaluate Formula and Error Checking', 'The two auditing tools that answer why a cell says what it says. Evaluate Formula underlines one part of the formula and replaces it with its value on each click, until the formula is the answer. Error Checking walks every cell that reports an error, with a sentence on what each one means, and every formula that breaks the pattern of the ones above and below it. Both are engine functions too: evaluationSteps() and checkSheet() run with no browser.', 'Spreadsheet', SheetAuditing483, { pro: true }),
  demo('484-sheet-cell-images', 'IMAGE: a picture inside the cell', 'Excel\'s IMAGE puts a picture IN a cell rather than floating one over it, so it sorts with its row, filters with it, copies as a formula and moves when the cells move. A product list whose thumbnail column reads the swatch beside it: sort by price and the pictures follow their rows. The source is a web address or a data URL, anything else stays text rather than becoming a broken image, and the second argument is the alt text a screen reader reads.', 'Spreadsheet', SheetCellImages484, { pro: true }),
