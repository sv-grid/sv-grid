...
import SchedulerUnified368 from '../demos/368-scheduler-unified-views.svelte'
import SchedulerContent369 from '../demos/369-scheduler-content-calendar.svelte'
import SchedulerShifts370 from '../demos/370-scheduler-shift-roster.svelte'
...
export const demos: Demo[] = [
  ...
  demo('364-scheduler-timegrid',    'Time-grid (week / day) + drag/resize', 'The Week and Day views lay overlapping events out per a switchable collisionMode (split / cap+more / stack). `editable` turns on drag-to-move and top/bottom edge resize; both snap to slotMinutes and fire onEventMove / onEventResize. Enterprise scheduler view.', 'Scheduler', SchedulerTimegrid364, { pro: true }),
  demo('365-scheduler-resources',   'Resource scheduling (rooms as columns)', 'Set scheduler.resourceField and EVERY time-grid view groups by resource: Day = one column per resource, Week = each resource across all 7 days (spanning group headers + horizontal scroll). A resource legend/filter toggles a resource in any view; groupByDate flips to date-major. Enterprise scheduler view.', 'Scheduler', SchedulerResources365, { pro: true }),
  demo('366-scheduler-recurring',   'Recurring events',            'A row can carry a recurrenceField (a RecurrenceRule) and the scheduler renders one event per matching day - reusing the same pure recurrence engine behind SvCalendar. Click an event for a recurrence editor (add / change / remove the repeat pattern); drag or resize a recurring event to edit the whole series. Calendar / Table switcher. Enterprise scheduler view.', 'Scheduler', SchedulerRecurring366, { pro: true }),
  demo('367-scheduler-agenda',      'Agenda view + event editor',  'The Agenda view is a chronological list grouped by day. Clicking an event opens the built-in detail drawer (scheduler.drawer -> SvDrawer + SvForm with the UI-kit editors); onEventCommit mirrors the edit back onto the row and re-colours it by priority. Enterprise scheduler view.', 'Scheduler', SchedulerAgenda367, { pro: true }),
  demo('368-scheduler-unified-views','One grid, three views (Table / Calendar / Kanban)', 'The SAME <SvGrid> + same rows rendered three ways by swapping one prop: a plain table, a Calendar (scheduler), or a Kanban board (board). Reschedule on the calendar, drag a card between lanes, or edit a cell - every change flows back to the one rows array so all three stay in sync. The "grid is the hero" story.', 'Scheduler', SchedulerUnified368, { pro: true }),
  demo('369-scheduler-content-calendar', 'Content calendar (multi-day campaigns)', 'An editorial Month calendar: single posts sit on a day while multi-day campaigns render as ONE continuous bar spanning the days they run and wrapping across week rows. Colour-coded by channel; double-click to add, click to edit, drag an edge to change the run. Enterprise scheduler view.', 'Scheduler', SchedulerContent369, { pro: true }),
  demo('370-scheduler-shift-roster', 'Staff shift roster (resources by person)', 'A weekly staff roster: resourceField groups the Week grid by team member so each person gets their own day columns. Drag a shift to another person or time to reassign, resize to change hours, and use the legend to focus on one person. Colour = role. Enterprise scheduler view.', 'Scheduler', SchedulerShifts370, { pro: true }),
