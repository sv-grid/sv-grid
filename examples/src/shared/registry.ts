...
import ExcelShortcuts452        from '../demos/452-excel-shortcuts.svelte'
import FormulaBarFormats453     from '../demos/453-formula-bar-formats.svelte'
import PasteFindStructure454    from '../demos/454-paste-find-structure.svelte'
import Workbook455              from '../demos/455-workbook.svelte'
import SalesReportWorkbook456   from '../demos/456-sales-report-workbook.svelte'
import GoalSeek457              from '../demos/457-goal-seek.svelte'
import DataCleanup458           from '../demos/458-data-cleanup.svelte'
import NamedRangesForecast459   from '../demos/459-named-ranges-forecast.svelte'
import ReviewCommentsProtection460 from '../demos/460-review-comments-protection.svelte'
import OrderIntakeValidation461  from '../demos/461-order-intake-validation.svelte'
import RegionalScorecardCf462    from '../demos/462-regional-scorecard-cf.svelte'
import MergedReportHeaders463    from '../demos/463-merged-report-headers.svelte'
...
import LoanModelFiles484       from '../demos/484-loan-model-files.svelte'
import SheetChartsObjects485   from '../demos/485-sheet-charts-objects.svelte'
import SheetSparklines486      from '../demos/486-sheet-sparklines.svelte'
import SheetPivotRange487      from '../demos/487-sheet-pivot-range.svelte'
import SheetCollaboration488   from '../demos/488-sheet-collaboration.svelte'
import SheetHyperlinks489      from '../demos/489-sheet-hyperlinks.svelte'
import SheetLetLambda490       from '../demos/490-sheet-let-lambda.svelte'
import SheetTables491          from '../demos/491-sheet-tables.svelte'
...
import BoxPlot433              from '../demos/433-chart-boxplot.svelte'
import AxesStyling434          from '../demos/434-chart-axes-styling.svelte'
import TypeGallery435          from '../demos/435-chart-type-gallery.svelte'
import SyncZoom436             from '../demos/436-chart-sync-zoom.svelte'
import FinancialWorkbench437   from '../demos/437-chart-financial-workbench.svelte'
import BarCharts438            from '../demos/438-chart-bar.svelte'
import LineCharts439           from '../demos/439-chart-line.svelte'
import AreaCharts440           from '../demos/440-chart-area.svelte'
import PieDonut441             from '../demos/441-chart-pie-donut.svelte'
import ScatterBubble442        from '../demos/442-chart-scatter-bubble.svelte'
import ComboChart443           from '../demos/443-chart-combo.svelte'
import Distribution444         from '../demos/444-chart-distribution.svelte'
import HeatmapCalendar445      from '../demos/445-chart-heatmap-calendar.svelte'
import RadarRadial446          from '../demos/446-chart-radar-radial.svelte'
import Hierarchy447            from '../demos/447-chart-hierarchy.svelte'
import Flow448                 from '../demos/448-chart-flow.svelte'
import WaterfallFunnel449      from '../demos/449-chart-waterfall-funnel.svelte'
import GaugeBullet450          from '../demos/450-chart-gauge-bullet.svelte'
import Annotations451          from '../demos/451-chart-annotations.svelte'
...
import ServerGroupingModel       from '../demos/496-server-grouping-model.svelte'
...
import CalendarBirthday260 from '../demos/260-calendar-birthday.svelte'
import FormDynamic323 from '../demos/323-form-dynamic.svelte'
import FormWizard324 from '../demos/324-form-wizard.svelte'
import FormArray325 from '../demos/325-form-array.svelte'
import DockPopout367 from '../demos/367-dock-popout.svelte'
import TreeData426 from '../demos/426-tree-data.svelte'
import GroupFooters427 from '../demos/427-group-footers.svelte'
import AsyncEditorOptions428 from '../demos/428-async-editor-options.svelte'
...
const baseDemos: Demo[] = [
  ...
  demo('260-calendar-birthday', 'Calendar - date of birth', 'SvCalendar as a date-of-birth picker: single selection with max clamped to today so future dates are blocked, opening on the decade (year) grid for fewer clicks to a birth year, with a live age readout. Same component SvGrid mounts to edit a date cell.', 'Date & Time', CalendarBirthday260),
  ...
  demo('323-form-dynamic', 'Dynamic form', 'SvForm with conditional fields: visible / disabled derived from other values (hidden fields skip validation and drop from the payload), async field validation, an async onSubmit with a loading button, and a Reset that restores the initial values.', 'Layout', FormDynamic323),
  demo('324-form-wizard', 'Wizard form', 'SvForm as a multi-step wizard: titled FormSection groups become validated steps with Back / Next (gated on each step) and a final async Submit. Drop the stepper prop and the same schema renders as stacked fieldsets.', 'Layout', FormWizard324),
  demo('325-form-array', 'Field array form', 'SvForm field array (type: array): a repeatable group with add / remove rows, per-row validation (each item validates against itself), required / minItems gating, and an array value in the submitted payload - an invoice line-items form.', 'Layout', FormArray325),
  ...
  demo('343-kanban-board',          'Kanban board mode',           'The same <SvGrid>, same data + columns, rendered as a Kanban board by setting one `board` prop: rows become cards in horizontal lanes bucketed by a `status` field. Built-in drag-and-drop AND keyboard move (Space + arrows) reassign the lane and reorder; double-click / F2 opens a built-in card editor; WIP limits, add-card, and a rich card snippet (assignee avatar + priority chip). Toggle Board / Table to see it is one grid.', 'Kanban', KanbanBoard343, { pro: true }),
  demo('344-kanban-sprint',         'Sprint board (swimlanes + WIP)', 'An agile sprint board: status lanes crossed with a swimlane per assignee, an enforced WIP limit on In progress, and a story-point roll-up in every lane header via board.laneSummary. Drag across a band to reassign the owner; double-click to edit. Collapsible lanes.', 'Kanban', KanbanSprint344, { pro: true }),
  demo('345-kanban-pipeline',       'Sales pipeline (deal board)', 'A CRM deal board: stages as lanes with a live total-value roll-up per stage (board.laneSummary). Rich deal cards; drag to advance a deal, double-click to edit. Moves and edits persist to localStorage via onCardMove / onCardCommit, so a reload restores the board.', 'Kanban', KanbanPipeline345, { pro: true }),
  demo('346-kanban-virtualized',    'Large board (virtualized)', 'Thousands of cards per lane kept smooth with board.virtualized - only the cards in view are in the DOM. A slider scales the board from 1k to 50k cards; drag-and-drop and keyboard move still work across the whole window.', 'Kanban', KanbanVirtualized346, { pro: true }),
  demo('347-kanban-support',        'Support triage board', 'A helpdesk triage board: status lanes crossed with a priority swimlane, a search box front-and-center, and an SLA colour bar on each card driven by ticket age. Drag across a band to re-triage a ticket priority.', 'Kanban', KanbanSupport347, { pro: true }),
  demo('348-kanban-content',        'Content calendar', 'An editorial pipeline: stage lanes crossed with a swimlane per channel. Rich cards carry a cover glyph, author and target date. Drag a piece down the pipeline, or across a band to move it to another channel; double-click to edit.', 'Kanban', KanbanContent348, { pro: true }),
  demo('349-kanban-subtasks',       'Cards with sub-tasks & badges', 'The default board card made rich by config alone: label / due / assignee badges, a sub-task checklist with a progress bar (expand to tick items or add more), and a quick-add composer at the bottom of each lane. Double-click a card for the BUILT-IN detail drawer (board.drawer -> SvDrawer + SvForm with the UI-kit editors); toggle all fields vs a customized subset. Board / Table switcher.', 'Kanban', KanbanSubtasks349, { pro: true }),
  demo('350-kanban-structure',      'Group-by switch + reorderable lanes', 'Board structure, live: switch the lane axis at runtime (Status / Assignee / Priority) and the board re-buckets, and drag a lane header to reorder the columns. Both built-in - the group-by switch resets card positions to the new field; lane order is tracked and persists with persistKey. Board / Table switcher.', 'Kanban', KanbanStructure350, { pro: true }),
  ...
  demo('352-kanban-epics',          'Epics -> stories (hierarchical cards)', 'Point board.childrenField at a row child rows and each card gains a children count that expands to show its sub-cards inline (Jira epic -> stories). Each child shows its own title + status. Board / Table switcher.', 'Kanban', KanbanEpics352, { pro: true }),
  ...
  demo('381-scheduler-app-calendar', 'Horizon - Outlook-style calendar app', 'A full calendar client on the Scheduler + SvDockManager: NavPane module rail + mini date-picker + calendar toggles, and a dockable workspace (drag / resize / float panes) with the calendar over an Upcoming grid and an Inbox. Recurring events with per-occurrence edits, a second time-zone ruler, iCal import/export and undo.', 'Scheduler', SchedulerAppCalendar381, { pro: true }),
  demo('382-scheduler-app-clinic', 'Meridian Clinic - appointment scheduling', 'A front-desk appointment console: providers with their own hours as day columns, a drag-in patient waitlist, a utilization gauge, and a live patient-flow pipeline (Waiting / Checked in / In room / Completed) that updates as you work.', 'Scheduler', SchedulerAppClinic382, { pro: true }),
  demo('383-scheduler-app-dispatch', 'Dispatch - field-service operations board', 'A map-first field-service operations console: a live ops map with technician pins by status and job markers, an unassigned queue you drag onto the crew timeline, and SLA countdowns. No double-booking.', 'Scheduler', SchedulerAppDispatch383, { pro: true }),
  demo('384-scheduler-app-roadmap', 'Portfolio roadmap - initiatives across squads', 'A dashboard-forward portfolio planner: a live portfolio-health strip (status mix, capacity by squad, delivery trend, on-track gauge) over a Gantt of squads x initiatives with progress bars and milestones.', 'Scheduler', SchedulerAppRoadmap384, { pro: true }),
  demo('385-scheduler-app-content', 'Broadcast - content & marketing calendar', 'A content command center: one grid, two views - a Calendar / Board toggle over the same posts. The Board is the approval pipeline (Idea / Draft / In review / Scheduled / Published) with per-card engagement sparklines.', 'Scheduler', SchedulerAppContent385, { pro: true }),
  ...
  demo('428-async-editor-options', 'Async editor options', 'editorOptions can return a Promise - for the whole column, or per row for a cascade. The dropdown shows Loading… while the request is in flight, results are cached (per column, or per column+row) so reopening never refetches, and api.refreshEditorOptions() invalidates when the server list changes.', 'Editing', AsyncEditorOptions428),
  ...
  demo('429-move-cells',            'Drag a range to move / copy','Grab the border of a selected block and drag it: the values move, or copy with Ctrl / Cmd held at drop time. Refused whole rather than half-applied when the destination is off-grid or read-only. Ctrl+Z walks it back.', 'Editing', MoveCells429),
  ...
  demo('427-group-footers', 'Group display modes + footers', 'Switch between groupRows banners, a single combined Group column, and one column per grouped field. groupFooters closes each group with a subtotal row under the real columns. Paging counts DATA rows, so pageSize means what it says: a page reprints the banners its rows sit under, and footers never eat the budget.', 'Sorting & Grouping', GroupFooters427),
  ...
  demo('426-tree-data', 'Tree data (hierarchy)', 'treeData nests rows by parent id into an expandable hierarchy. Tree rows stay real data rows - own cells, formatting, editing - and just gain an expander plus indent. Takes flat parent-id data directly, or nested children arrays via flattenTreeData. Full treegrid a11y with arrow-key expand.', 'Sorting & Grouping', TreeData426),
  ...
  demo('430-selection-bar',         'Bulk-action bar',             'selectionBar floats a bar over the grid while rows are selected: the count, your actions, and Clear. Actions get the selected rows in display order; hidden / disabled re-check against the live selection, so Merge appears at 2+ rows and Mark done greys out once everything picked is closed.', 'Selection & Clipboard', SelectionBar430),
  ...
  demo('433-chart-boxplot',         'Box plot + error bars',       'Distribution rather than average: box plots with the 1.5 IQR whisker rule and individual outliers, next to the same data as a bar chart of the means with error bars. two regions with nearly identical means sit side by side in the bars and look nothing alike in the boxes. boxStats() summarises a raw sample, rowsToBoxSpec() does it per group, and ChartSeries.errors annotates any existing mark without changing its type.', 'Charts', BoxPlot433),
  demo('434-chart-axes-styling',    'Axes, titles and styling',   'The chart hands the decisions back: a numeric x axis that spreads payload sizes the way the numbers do, pinned domains with a fixed tick interval and a formatter per axis, grid lines on or off, a shaded budget band, a vertical reference line, per-point markers and colours, a stepped dashed series on the right axis, a title / subtitle / caption, a legend on any side, and a tooltip snippet in single-series mode. Below it, 50,000 monitoring readings decimated to one point per pixel with LTTB or min / max - toggle it off to see the cost.', 'Charts', AxesStyling434),
  demo('435-chart-type-gallery',    'Every chart type',           'Every chart type from one dataset, thirty live thumbnails: bar, horizontal bar, line, area, lollipop, dumbbell, range bar, range area, pareto, radial column, radial bar, nightingale, pie / donut, tree map, sunburst, funnel, waterfall, sankey, chord, radar, heat map, scatter, box plot, histogram, gauge, bullet, calendar, stream, candlestick and OHLC. Click a card for the full-size chart and its variants: classic / hollow / Heikin-Ashi candles, funnel / pyramid / cone, wiggle / silhouette streams, stacking. Every spec is built with the helpers the grid chart panel uses.', 'Charts', TypeGallery435),
  demo('436-chart-sync-zoom',       'Synchronized charts with zoom, pan and presets', 'Two years of daily prices and volumes as two charts that move as one: hover either for a shared crosshair, Ctrl + wheel or pinch to zoom around the pointer, Shift + drag or the hand button to pan, 1W / 1M / 3M / 6M / YTD / 1Y / All presets, and a grid that shows exactly the rows in the window. Right-click for a context menu with export, zoom and series items plus your own. Below: bars that grow in and slide on every data change with multi-selection, and a sunburst that drills down on click with a breadcrumb back up.', 'Charts', SyncZoom436),
  demo('437-chart-financial-workbench', 'Financial workbench', 'A year of daily prices as a trading terminal, free in the grid package: rows to candles with rowsToOhlcSpec, a daily / weekly toggle through resampleOhlc, Bollinger bands or an EMA on the price, volume, RSI, MACD, stochastic, ATR and OBV panes stacked under it with one x axis and one crosshair, a last-price pill, earnings and dividend flags with tooltips, drawing tools (trend line, ray, Fibonacci, rectangle, arrow, note) stored as data points, range presets, Ctrl + wheel zoom, PDF and print. Below, the same rows in the grid panel with Candlestick picked, indicator chips, the chart builder and the link toggle.', 'Charts', FinancialWorkbench437),
  demo('438-chart-bar', 'Bar charts', 'Eight quarters of revenue for three regions as grouped, stacked, 100% and horizontal bars from one spec: a year tier over the quarters through categoryGroups, a plan line as a pill on the value axis, data labels that step aside when the bars get narrow, and a second chart of variance to plan where negative bars hang from zero in the colour of their sign.', 'Charts', BarCharts438),
  demo('439-chart-line', 'Line charts', 'Twenty-four months of sign-ups for three plans on a real time axis, plus a six-month forecast: smooth or stepped lines, markers per series, a gap where two months of data are missing or a bridge across it (connectNulls), a dashed forecast series that meets the actuals at a reference line for today, a shaded outage band, series names at the line ends, crosshair pills, and a log scale for plans an order of magnitude apart.', 'Charts', LineCharts439),
  demo('440-chart-area', 'Area charts', 'Sessions by channel over a year as areas: stacked, in two independent stack groups on one chart, normalised to a share, or as a stream on a centred baseline with the wiggle and silhouette offsets, with gradient fills. Below it a range area: the daily temperature band between the low and the high with the mean as a line and the freezing point shaded.', 'Charts', AreaCharts440),
  demo('441-chart-pie-donut', 'Pie, donut and sunburst', 'Twelve browsers\' share of traffic as the part-of-a-whole family: a pie that becomes a donut through innerRadius, callout labels on leader lines or shares on the slices, small slices folded into Other with topN, a legend that hides and isolates, a slice click that filters the grid beside it, and a sunburst of the same numbers nested under their engine that drills in on click.', 'Charts', PieDonut441),
  demo('442-chart-scatter-bubble', 'Scatter and bubble charts', 'Ninety products as points, marketing spend against revenue, one series per segment: a third value as the bubble size, a regression per series (linear, quadratic, exponential, logarithmic or power) fitted on x with its equation and R-squared in the tooltip, quadrant lines at the averages, a log x axis for spend that spans two orders of magnitude, and point selection that reaches the grid.', 'Charts', ScatterBubble442),
  demo('443-chart-combo', 'Combination charts', 'Revenue and cost as bars, gross margin as a smooth line on its own right-hand axis formatted as a percent, a margin target drawn on that axis, cost switchable to an area, a margin line that is smooth or straight, data labels on the revenue bars only through a formatter, and a shared tooltip that lists every series for the hovered month.', 'Charts', ComboChart443),
  demo('444-chart-distribution', 'Histograms and box plots', 'Twelve hundred request latencies from three services as the two charts that show a distribution: a histogram with a bin slider where the three series share the same edges, stacked or side by side, with a p95 marker; and box plots per service and region with an adjustable whisker rule, outlier dots, and a log scale for a service an order of magnitude slower.', 'Charts', Distribution444),
  demo('445-chart-heatmap-calendar', 'Heat maps and calendars', 'Support tickets by hour and weekday as a heat map matrix with three colour ramps (sequential, diverging for a change against last week, custom stops), values in the cells, and cell selection; and a year of days as a calendar heat map, one square per day laid out in weeks with the months labelled and the range pinned.', 'Charts', HeatmapCalendar445),
  demo('446-chart-radar-radial', 'Radar and radial charts', 'Three phones scored on six attributes as a radar chart with the axis pinned to 0 to 100 and each product switchable on and off, and a year of sales as the radial family: stacked radial columns around a circle, a nightingale where the radius carries the value, and radial bars read against a shared scale. The same categories and series a bar chart takes; only the type changes.', 'Charts', RadarRadial446),
  demo('447-chart-hierarchy', 'Tree maps and sunbursts', 'A portfolio as a hierarchy, sectors then holdings, sized by market value: a tree map whose holdings are coloured by the day\'s move on a red-to-green ramp or by sector, with the sector label over its block and a click that selects a holding; and the same tree as a sunburst that zooms into the ring you click with a breadcrumb back.', 'Charts', Hierarchy447),
  demo('448-chart-flow', 'Sankey and chord diagrams', 'Where things go: energy from five sources through two carriers to four uses as a three-column sankey whose ribbon width is the value, and trade between five regions as a chord diagram, the shape for flows that run both ways between the same arcs. Hover a ribbon for its value, a node for its total.', 'Charts', Flow448),
  demo('449-chart-waterfall-funnel', 'Waterfall, funnel and pareto', 'Three charts that explain how a total came to be: a waterfall that bridges revenue to net income with subtotals that restart from zero, connectors between the steps and named colours for up, down and total; a sign-up funnel with the conversion and drop-off per step in the tooltip and a trapezoid, pyramid or cone shape; and a pareto of return causes with its cumulative line and the 80% mark.', 'Charts', WaterfallFunnel449),
  demo('450-chart-gauge-bullet', 'Gauges and bullet charts', 'A KPI strip: three gauges for uptime, p95 latency and error rate with coloured bands behind the needle, a target tick and a unit, driven by sliders so the needle sweeps to the new value; and six KPIs as bullet charts, each a bar against its target tick over poor, fair and good ranges, the compact way to put six targets on one screen.', 'Charts', GaugeBullet450),
  demo('451-chart-annotations', 'Annotations, reference marks and notes', 'Five years of monthly active users with the story drawn on the chart: flags for each launch, a pin for the outage and a dot for the pricing change with a text each in the tooltip, shaded bands for the recession on the x axis and the target on the y axis, a pill for the all-time high, a trend line, a rectangle and a note as drawings that travel with the data, an Annotate toggle for the reader\'s own notes, drawing tools in the toolbar, and a tooltip that pins on a click.', 'Charts', Annotations451),
  ...
  // ----- Spreadsheet
  demo('27-spreadsheet-ribbon',     'Spreadsheet shell (SvSheet)', 'The whole Excel surface as one component: <SvSheet workbook={wb} />. Ribbon (Home / Insert / Formulas / Data), a Name Box and fx bar showing the raw formula behind the active cell, A..I headers over the 1..N row gutter, sheet tabs, and a Sum / Average / Count status bar. Ribbon buttons and keyboard shortcuts run the same calls, so clicking a bold cell lights the Bold button and Ctrl+B puts it out.', 'Spreadsheet', SpreadsheetRibbon, { pro: true }),
  demo('83-spreadsheet-formulas',   'Spreadsheet + formulas',      'The formula engine with the ribbon turned off so nothing competes with it: the fx bar shows =SUM(B2:D2) while the grid shows 56,600, function names autocomplete with a signature hint, absolute refs hold through a fill, and recalculation walks the dependency graph rather than sweeping the sheet. ~50 functions including INDEX/MATCH, VLOOKUP, COUNTIF and TEXTJOIN, with cycle detection.', 'Spreadsheet', SpreadsheetFormulas, { pro: true }),
  ...
  demo('170-cell-merging',          'Cell merging (spreadsheet shell)','A real invoice rendered on an Excel-style shell: A / B / C / D / E column letters across the top, row numbers down the left. Brand band, bill-from / bill-to address blocks, meta block, line items, totals, notes, signatures - all assembled from MergeSpec + CellBorderSpec. Editable Qty / Rate / addresses / notes; totals recompute live.', 'Spreadsheet', CellMergingDemo, { pro: true }),
  demo('173-hyperformula',          'HyperFormula integration',    'Full HyperFormula engine wired into the grid as a peer-optional dep. Editable spreadsheet with A1-style cell refs, dozens of formulas across math (SUM / SUMIF), lookup (VLOOKUP / INDEX-MATCH), text (CONCAT / UPPER), date (TODAY / DATEDIF), logical (IF nests), financial (PMT / IRR / NPV), statistical (AVERAGE / MAX / RANK).', 'Spreadsheet', HyperFormulaDemo, { pro: true }),
  demo('207-blank-sheet',           'Blank sheet - just type',     'An empty workbook and nothing else: <SvSheet /> with no props at all. Click a cell and type a number or =SUM(A1:A5). Every Excel shortcut is already live without a line of wiring: Ctrl+Arrow to the edge of a block, Ctrl+D / Ctrl+R to fill, Ctrl+; for today, Alt+= for AutoSum, Ctrl+Shift+4 for currency, and Ctrl+Z undoing a whole action rather than one cell.', 'Spreadsheet', BlankSheet, { pro: true }),
  ...
  demo('452-excel-shortcuts',       'Excel keyboard shortcuts',    'The muscle memory a spreadsheet user arrives with: Ctrl+Arrow jumps to the edge of the data region (a run-boundary search, so it hops gaps rather than running to the end), Ctrl+Shift+Arrow extends there, Ctrl+A takes the current region then the sheet, Ctrl+Space / Shift+Space take the column or row, Ctrl+D / Ctrl+R fill, Ctrl+; stamps the date. One enableSheet() call. Ctrl+Z undoes a whole fill in one press, not one per cell.', 'Spreadsheet', ExcelShortcuts452, { pro: true }),
  demo('453-formula-bar-formats',   'Formula bar + cell formats',  'The Excel cell experience: a formula bar showing the RAW text behind the active cell (the grid shows 1,234.50, the bar shows =B2*C2) with function autocomplete and signature hints, a Name Box that jumps to an address, and number formats that live on the CELL rather than the column. Ctrl+Shift+4 for currency, Ctrl+B to bold. The format store keys on row id, so sorting does not leave formatting behind on the old index.', 'Spreadsheet', FormulaBarFormats453, { pro: true }),
  demo('454-paste-find-structure',  'Paste Special, Find/Replace', 'The three Excel operations that write more than one cell: Paste Special (values / formulas / formats / transpose / arithmetic, with a text/html clipboard flavour so formats survive a round trip through Excel), Find and Replace that searches what you SEE but writes what you TYPED so a formula is not destroyed by replacing its result, and insert/delete that rewrites every formula - inserting above =SUM(B1:B4) widens it instead of dropping the new row.', 'Spreadsheet', PasteFindStructure454, { pro: true }),
  demo('455-workbook',              'Workbook (multi-sheet)',      'Three sheets and one formula engine spanning them: Prices is a lookup table, Orders VLOOKUPs into it, Summary SUMs over Orders. Edit a unit price and the change travels two sheets, recomputing only what depended on it. Tabs switch, rename and reorder; Ctrl+PageUp/PageDown and Shift+F11 work from anywhere in the grid.', 'Spreadsheet', Workbook455, { pro: true }),
  demo('456-sales-report-workbook', 'Sales report workbook', 'A three-sheet workbook on the Excel shell: Orders with XLOOKUP prices and IF discounts, a Products price list, and a Summary where every figure is live - COUNTIF / SUMIF by region, share of a named total, RANK, per-rep attainment with an IFS status, INDEX / MATCH for the best region. Edit an order and the whole page follows across sheets. Defined names in the Name Box; bands, currency and percent formats ship with the document.', 'Spreadsheet', SalesReportWorkbook456, { pro: true }),
  demo('457-goal-seek', 'What-if analysis: Goal Seek', 'A pricing model with three scenarios side by side and Excel\'s Goal Seek over it: Data -> Goal Seek, set the profit cell to 20,000 by changing the price, and the solver (secant with a bisection fallback) finds it. The sheet does not move until the status dialog says "found a solution" and you press OK; OK writes through the grid\'s command context, so it is one Ctrl+Z.', 'Spreadsheet', GoalSeek457, { pro: true }),
  demo('458-data-cleanup', 'Data cleanup: Text to Columns, Remove Duplicates', 'A CRM export landed in column A, semicolon-separated, with the same people in it twice. Data -> Text to Columns opens a wizard that has already guessed the delimiter and previews the split; Data -> Remove Duplicates lets you tick the columns that decide identity, compares case-insensitively like Excel, and reports "3 duplicate values found and removed; 11 unique values remain". Each operation is one Ctrl+Z.', 'Spreadsheet', DataCleanup458, { pro: true }),
  demo('459-named-ranges-forecast', 'Defined names + Name Manager', 'An assumptions-driven twelve-month forecast where every formula reads a defined name - =B2*(1+Growth-Churn) - instead of an address. Pick a name in the Name Box to jump to its cell on the Assumptions sheet; open Formulas -> Name Manager to add, repoint or delete names and watch the forecast recompute. Inputs in blue, red negatives in parentheses, break-even found with INDEX / MATCH.', 'Spreadsheet', NamedRangesForecast459, { pro: true }),
  demo('460-review-comments-protection', 'Cell comments + Protect Sheet', 'An expense sheet under review on the Excel shell. It opens protected: every cell is locked except the reviewer\'s Receipt and Status columns, so a keystroke on an amount is refused and the status bar says why. Three reviewer notes sit on the cells they are about (Excel\'s red corner): hover, Shift+F2, Review > Next, Show All Comments. Status is a list, the totals by status are SUMIFs, and protection, notes and rules all ride in the document.', 'Spreadsheet', ReviewCommentsProtection460, { pro: true }),
  demo('461-order-intake-validation', 'Order intake: data validation', 'An order form with Excel\'s Data Validation on it: Region and Product are lists (the arrow or Alt+Down drops them; Product reads the price list on a second sheet), Qty is a whole number from 1 to 500 with a Stop alert and Retry, Discount is a decimal up to 20% with a Warning you can keep, Ship by is a date on or after the order date through a relative bound (=A2 moves with the row). Unit price is an XLOOKUP, the total follows.', 'Spreadsheet', OrderIntakeValidation461, { pro: true }),
  demo('462-regional-scorecard-cf', 'Regional scorecard: conditional formatting', 'Eight regions, three months, a target, and every colour on the page is a conditional formatting rule over computed values: Highlight Cells on attainment (under 90% red, 100% and over green), a Data Bar and a Top 3 on the quarter, a Green - Yellow - Red Color Scale across the months, an Icon Set on the trend. Edit a month and the arrow, the bar, the fill and the rank all move; Manage Rules lists the six in priority order.', 'Spreadsheet', RegionalScorecardCf462, { pro: true }),
  demo('463-merged-report-headers', 'Budget report: merged headers', 'A half-year budget report laid out with merged cells the way Excel lays one out: a title merged across the page, Q1 / Q2 / H1 group headers merged over their months, a Line corner merged down two rows, a notes paragraph merged into one wrapped block. A merge is one cell to the grid: click inside Q1 and the Name Box says B2, Merge & Center lights, the arrows step over it and its column letters are shaded. Unmerge and merge again from the ribbon; Ctrl+Z each step.', 'Spreadsheet', MergedReportHeaders463, { pro: true }),
  demo('464-ticket-log-autofilter', 'Support ticket log: AutoFilter', 'Forty support tickets with Excel\'s Filter on the header row, opened already filtered to what is still open: the funnel on Status, blue row numbers, "N of 40 records found" in the status bar. The arrows drop Excel\'s menu: sort, Clear Filter, Text and Number Filters with two conditions, a search box, (Select All) and the values with counts. The rows are worked out again after every edit, so a ticket typed Closed folds away at once. Ctrl+Shift+L toggles it.', 'Spreadsheet', TicketLogAutofilter464, { pro: true }),
  demo('465-autosave-document', 'Autosave: a document that survives a reload', 'A project tracker that keeps itself in localStorage: onChange fires once per tick with every kind of change, the demo debounces it and writes getState() as JSON, and the next visit rebuilds the document with createSheetDocument({ state }) and hands it to <SvSheet document>. Add a comment, bold a row, hide a column, type a task, reload: it is all back, rules and merged title and frozen rows included. Reset puts the shipped document back with setState().', 'Spreadsheet', AutosaveDocument465, { pro: true }),
  demo('466-paste-from-excel', 'Paste from Excel: formats and formulas survive', 'Excel puts an HTML document on the clipboard beside the tab-separated text: formats in a style block keyed by class, formulas in x:fmla, raw numbers in x:num. The sheet reads it, so a pasted block arrives bold, filled, with its number formats, and with its formulas moved to where they landed; Google Sheets\' data-sheets-formula flavour reads the same. Two buttons put exactly what Excel and Sheets put on the clipboard; click a cell and Ctrl+V.', 'Spreadsheet', PasteFromExcel466, { pro: true }),
  demo('484-loan-model-files', 'Loan model: PMT, accounting formats and the File tab', 'A mortgage model built from the sheet\'s own financial functions: PMT for the payment, IPMT and PPMT for each period\'s split, NPER and RATE for the term and rate that fit, SUMPRODUCT for the first year of interest. The money cells wear the Accounting format, Extra Payment carries a validation rule with an Input Message, and Circle Invalid Data rings what breaks it. The File tab is the point: Save As writes the whole model as an .xlsx Excel opens with its formulas, formats and rules, Save As ODS writes the same model for LibreOffice Calc and Save As XLS the Excel 97-2003 binary, Open reads any of them (or a .csv) back by their bytes rather than their name, and Export CSV takes the active sheet.', 'Spreadsheet', LoanModelFiles484, { pro: true }),
  demo('485-sheet-charts-objects', 'Charts and pictures on the sheet', 'Charts anchored over the cells the way Excel anchors one: each reads a range rather than a copy of the numbers, so typing into a cell redraws it. Insert > Chart charts the selected block and reads its first row and column as the labels, Insert > Picture puts an image on the sheet, and a double-click opens the Chart dialog for the type, the title, series in columns or rows and stacking. Drag an object to move it, its corner to resize, Delete to remove. It hangs from a cell, so inserting a row above moves it, and it rides in getState() and in the .xlsx as a real drawing part.', 'Spreadsheet', SheetChartsObjects485, { pro: true }),
  demo('486-sheet-sparklines', 'Sparklines: a chart inside the cell', 'Excel\'s smallest chart, and not an object: a sparkline IS the cell. One per row of a block of numbers, drawn from the range rather than a copy, so editing a number redraws it. Insert > Sparklines offers Line, Column and Win/Loss, with Edit for the group\'s ranges, kind and colours and Clear for the groups the selection touches; selecting a cell that holds one turns the kind buttons into a change to that group. They are kept per group the way Excel keeps them, ride in getState() and in the .xlsx where Excel keeps them, and move with an insert or a delete.', 'Spreadsheet', SheetSparklines486, { pro: true }),
  demo('487-sheet-pivot-range', 'PivotTable from a range', 'Excel\'s Insert > PivotTable over a block of cells, on the same pivot engine the grid uses for its own pivot mode. The sheet keeps the definition - the source block, where the result goes, and which field is a row, a column or a measure - and writes the result as plain cells in one undo, so it can be formatted, charted, printed and saved to an .xlsx like any other block. Show Details writes the source rows behind a cell to a sheet of their own, and Refresh rebuilds it from the source; opening the dialog from inside one edits it.', 'Spreadsheet', SheetPivotRange487, { pro: true }),
  demo('488-sheet-collaboration', 'Two people on one sheet: the delta stream', 'Two full spreadsheets over two separate documents, wired to each other by createDeltaStream: type in either and the other follows. What crosses the wire is a delta rather than the document, and the log shows each one as it goes - a formula travels as its text so the other side works out its own answer, an insert travels as the edit so both rewrite their own formulas, a format travels as the one part of the one sheet that changed. Presence rides the same wire and is never part of the document: each window draws the other\'s selection as a coloured box with a name on it. Conflicts are last writer wins, per cell. A real app puts a socket in the middle.', 'Spreadsheet', SheetCollaboration488, { pro: true }),
  demo('489-sheet-hyperlinks', 'Hyperlinks: Insert > Link and HYPERLINK', 'Excel\'s two ways of putting a link in a cell. Insert > Link (Ctrl+K) puts one ON the cell, so editing the text keeps it and clearing the cell takes it away; the HYPERLINK function puts one in a formula with a friendly name. A target that reads like an address moves the selection instead of leaving the page, so a cell links to another sheet; anything with a scheme opens in a new tab. A single click follows, a drag selects. Links ride in getState(), move with an insert, and go into the .xlsx both ways.', 'Spreadsheet', SheetHyperlinks489, { pro: true }),
  demo('490-sheet-let-lambda', 'LET and LAMBDA', 'The two modern Excel functions that turn a formula into something you can read, and the helpers that make a lambda worth writing. LET names a value inside the formula so it is written once and read by name; LAMBDA is a function written in the sheet, bound by LET and called by name, or called where it stands. MAP, BYROW, BYCOL, REDUCE, SCAN and MAKEARRAY put one over every cell, every row, every column, a fold, a running total, and an array built from its own indexes. They spill, and they go into the .xlsx under the prefix Excel stores them with.', 'Spreadsheet', SheetLetLambda490, { pro: true }),
  demo('491-sheet-tables', 'Format as Table and structured references', 'Excel\'s Ctrl+T: a block becomes a table with a header row, the banded look, filter arrows and a name its columns are read by, so a formula says Orders[Amount] instead of E2:E13 and keeps meaning it as rows are added. [@Qty] is this row\'s cell. Auto-expand is the point: typing under the last row grows the table, and every total that reads it grows too, with no formula re-pointed. Insert > Table Styles picks the look from eighteen presets named the way Excel names them. Tables ride in getState() and go into the .xlsx as real table parts, style included.', 'Spreadsheet', SheetTables491, { pro: true }),
  demo('492-sheet-iterative', 'Iterative calculation', 'A circular reference is normally an error, and every cell in the loop shows #CYCLE!. Some models are circular on purpose: a bonus that is a share of the profit it is taken out of, interest charged on the balance it is part of. Formulas > Calculation Options turns on iterative calculation, and the loop runs until it stops moving or the passes run out. The setting rides in getState() and goes into the .xlsx as calcPr, so the file opens the same way in Excel.', 'Spreadsheet', SheetIterative492, { pro: true }),
  demo('493-sheet-auditing', 'Evaluate Formula and Error Checking', 'The two auditing tools that answer why a cell says what it says. Evaluate Formula underlines one part of the formula and replaces it with its value on each click, until the formula is the answer. Error Checking walks every cell that reports an error, with a sentence on what each one means, and every formula that breaks the pattern of the ones above and below it. Both are engine functions too: evaluationSteps() and checkSheet() run with no browser.', 'Spreadsheet', SheetAuditing493, { pro: true }),
  demo('494-sheet-cell-images', 'IMAGE: a picture inside the cell', 'Excel\'s IMAGE puts a picture IN a cell rather than floating one over it, so it sorts with its row, filters with it, copies as a formula and moves when the cells move. A product list whose thumbnail column reads the swatch beside it: sort by price and the pictures follow their rows. The source is a web address or a data URL, anything else stays text rather than becoming a broken image, and the second argument is the alt text a screen reader reads.', 'Spreadsheet', SheetCellImages494, { pro: true }),
  demo('498-sheet-bound-tabs', 'A tab that is a table, and rows that fold away', 'A workbook tab that holds RECORDS rather than cells: Orders renders as the data grid, with its own headers, sorting, filtering and inline editing, beside ordinary cell sheets. Its records are projected into the workbook\'s cells, so the Summary tab reads it with plain formulas - SUM, SUMIF, SUMPRODUCT and VLOOKUP all reach across, because the engine is never told the tab is different. Summary also shows Data > Group: three regional blocks folded under their subtotals, with the level buttons at the corner of the outline bar.', 'Spreadsheet', SheetBoundTabs498, { pro: true }),
  demo('499-sheet-engineering-stats', 'Engineering, statistics, and the locale you type in', 'Four function families at once, one real example of each: a bearing from ATAN2 and DEGREES, a register mask in binary and hex with BITAND, an orchard queried through Excel\'s criteria-block grammar with DSUM, DCOUNT, DAVERAGE and DGET, and NORM.DIST with its inverse, a confidence interval and the chi-squared and t tails. The picker switches how numbers and formulas are SPELLED while the document keeps the invariant spelling, and column H is drawn as checkboxes whose ticks an ordinary COUNTIF counts.', 'Spreadsheet', SheetEngineeringStats499, { pro: true }),
  ...
  demo('46-scheduler',              'Clinic day scheduler',        'Clinic day roster on the enterprise Scheduler view: providers as resource columns on an hour axis; drag to reschedule / resize / reassign across providers, click to edit in the drawer, live now-line.', 'Industry Templates', Scheduler, { pro: true }),
  ...
  demo('52-pivot-table',              'Pivot - Table + Designer',       'Drag-and-drop Pivot Designer with Filters / Rows / Columns / Values zones, multi-level column headers, subtotal + grand-total rows, row-header sort menu.', 'Pivot Grid', PivotTable,             { pro: true }),
  ...
  demo('124-pivot-olap',              'Pivot - OLAP cube',              'Full BI dashboard around an OLAP cube: page header with crumbs + last-refresh + export, 5 KPI tiles with QoQ sparklines, left slicer rail (region multi-select, year picker, country search, view-mode, density, heatmap toggle), the cube in Tabular form (one column per row dim), right insights rail (top YoY movers, top contributors, notes).', 'Pivot Grid', PivotOlap,              { pro: true }),
