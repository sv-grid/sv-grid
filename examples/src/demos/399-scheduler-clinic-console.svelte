...
<section class="cx">
  ...
  <div class="cx-main">
        ...
        <SvDockManager bind:workspace minSize={120}>
          {#snippet pane(p)}
            ...
            {:else if p.id === 'upcoming'}
              <div class="cx-pane"><SvGrid data={rows} columns={columns} getRowId={(r) => r.id} sortable enableInlineEditing showPagination={false} rowHeight={30} containerHeight="100%" fitColumns /></div>
            {/if}
          {/snippet}
        </SvDockManager>
      </div>
    {:else if activeModule === 'patients'}
      ...
      <div class="cx-plain"><SvGrid data={patients} columns={patientColumns} getRowId={(r) => r.id} sortable filterable showPagination={false} containerHeight="100%" fitColumns /></div>
