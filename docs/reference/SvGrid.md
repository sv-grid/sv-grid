...
| `columnResize`       | `boolean`            | `false`     | Drag a column edge to change its width, or focus the handle and use Left/Right (Shift = 1px). **Off by default**, so `width` is the width you asked for. Grid-wide, not per-column. `autosizeColumn` / `fitColumns` work either way. Handles load on demand. |
| `rowResize`          | `boolean`            | `false`     | Drag a row's bottom edge to change its height, or focus the grip and use Up/Down (Shift = 1px). The grid remembers the heights, so no `rowHeight` function is needed. **Turns on `showRowNumbers`**, since the grip lives in the row header; an explicit `showRowNumbers={false}` wins and the grip falls back to the row's first cell. Ignored under `autoRowHeight`. Grips load on demand. |
