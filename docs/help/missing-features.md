...
| ~~Open and save xlsx~~ | **shipped** - the shell's File tab opens an .xlsx into the document and saves the document as one, formulas, formats, sizes, hidden lines and sheets, panes, merges, validation, conditional formatting, protection, comments and names included; `documentToXlsx` / `documentFromXlsx` in `@svgrid/enterprise/sheet` | done |
