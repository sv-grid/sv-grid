...
The `sv-grid-pro` package is soft-gated; you can evaluate every
...
`jszip` and `pdfmake` are lazy-loaded by sv-grid-pro - they're only
...
- The first time you actually invoke a Pro feature (export, import,
...
That's the integration. The Community grid is unchanged; Pro
...
Each Pro feature has a fully-working demo in the gallery
...
Subpath imports (`sv-grid-pro/export`, `sv-grid-pro/pivot`, etc.)
...
- [Pro licensing](./licensing.md) - what each tier covers
- [Pro support](./support.md) - what you get with each tier
