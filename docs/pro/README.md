...
setLicenseKey('SVPRO-…')   // once, at app startup
const pro = installPro(api) // wraps a SvGridApi with Pro methods
```

That's the whole integration. Every Pro helper hangs off the api object
...
For dev builds, the demos use `setLicenseKey('SVPRO-DEV-DEMO')` to
...
`installPro` mutates and returns the same api object - existing
references keep working. The `ProGridApi` type extends `SvGridApi` so
the grid's existing methods are still there alongside the new ones.

## Tree-shaking

Each Pro module is a separate subpath export:

```ts
import { exportGrid }     from 'sv-grid-pro/export'    // export only
import { importData }     from 'sv-grid-pro/import'    // import only
import { aiFilter }       from 'sv-grid-pro/ai'        // AI only
...
import parser don't ship. The `sv-grid-pro` barrel import is
...
```bash
pnpm add sv-grid-pro jszip            # community + Pro + xlsx
pnpm add sv-grid-pro jszip pdfmake    # also pdf
```
