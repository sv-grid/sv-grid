...
| Does the library include third-party trackers?    | **No.** Verify the bundle - 49 kB gzip, no analytics SDK.      |
...
| Are dependencies vetted?                          | Yes - 0 runtime dependencies in `sv-grid-community`. `sv-grid-pro` lazy-loads `jszip` + `pdfmake` as peers. See [security](../help/security.md) for the dep table. |
