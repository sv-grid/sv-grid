...
Install `sv-grid-community`, import `SvGrid`, and pass `data` and `columns` - a working, accessible grid is about fifteen lines. Add features (sorting, filtering, pagination) by registering them with `tableFeatures`.
