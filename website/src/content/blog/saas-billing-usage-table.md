...
### How do I format currency correctly in a billing grid?

Set `format: { type: 'currency', currency: 'USD' }` on the column rather than formatting in an accessor. The amount then displays localized but sorts and totals as a real number - essential for money.

### How do I show trustworthy totals in a billing table?

Use a summary footer and grouped subtotals computed over the filtered rows, so totals match what the user sees. Avoid averaging pre-averaged rates; sum the values and divide by the count for blended figures.
