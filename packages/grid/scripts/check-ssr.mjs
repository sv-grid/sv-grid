...
// The footer summary row used to server-render as empty cells: the totals were
// assigned from an $effect, and effects do not run during SSR. So the row
// occupied space and drew its border, then filled in after hydration - and
// stayed blank forever with JS off. They are derived now, so the numbers ship
// with the HTML.
//
// The row is opt-in (it used to default on, which gave every plain grid a
// totals row nobody asked for), so this renders a second grid with `summary`
// rather than relying on the default.
const summaryHtml = mod.ssr(two, { summary: true })
check('the summary row server-renders its totals, not empty cells', /sv-grid-summary-column[^>]*>\s*(Count: 2|1843|3795)/.test(summaryHtml) || summaryHtml.includes('Count: 2'))
check('summary totals are the REAL aggregate (sum of the year column)', summaryHtml.includes(String(1843 + 1952)))
check('a plain grid has NO summary row (it is opt-in)', !html.includes('sv-grid-summary-row'))
