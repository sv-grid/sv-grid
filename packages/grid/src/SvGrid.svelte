...
>
  ...
  // The editor UI is ~520 lines of template that only ever renders while a cell
  // is being edited, and editing is off by default - so it loads on demand.
  // Keyed on `editingEnabled`, NOT on `editingCell`: the module has to be
  // resident before the first double-click, or the first edit drops a frame.
  let CellEditor = $state<typeof import("./SvGridCellEditor.svelte").default | null>(null);
  $effect(() => {
    if (CellEditor) return;
    // `editingEnabled` covers the normal case and loads at mount. The other two
    // cover a consumer flipping `editable` on and starting an edit in the same
    // tick, which would otherwise leave the cell showing its read-only value.
    if (ctrl.editingEnabled || ctrl.editingCell || ctrl.fullRowEdit)
      import("./SvGridCellEditor.svelte").then((m) => (CellEditor = m.default));
  });
  $effect(() => {
    const menuOpen =
      ctrl.tooltip ||
  ...
  const onRowPointerDown = $derived(ctrl.onRowPointerDown);
  ...
  function rowDragAttrs(rowIndex: number) {
    ...
    return {
      ...
      // Touch fires no HTML5 drag events at all (#66), so it gets its own
      // entry point. It feeds the same drop indicators and commits through the
      // same path, and ignores mouse/pen so the drag events above still own those.
      onpointerdown: (e: PointerEvent) => onRowPointerDown(e, rowIndex),
    };
  }
