...
async function getJSZip(): Promise<any> {
  if (typeof window === 'undefined') {
    throw new Error('sv-grid-pro: importData requires a browser environment')
  }
  ...
  if (!jszipPromise) {
    jszipPromise = (async () => {
      ...
      try {
        ...
        throw new Error(
          'sv-grid-pro: xlsx import requires the "jszip" peer dependency. ' +
...
async function parseXlsx(file: File | Blob): Promise<string[][]> {
  ...
  if (!sheetEntry) {
    throw new Error(
      'sv-grid-pro: could not locate sheet1.xml in the .xlsx archive',
    )
  }
...
>(
  ...
  if (format === 'xlsx') {
    if (typeof opts.file === 'string') {
      throw new Error(
        'sv-grid-pro: xlsx import expects a File or Blob, not a string. ' +
...
function jsonToMatrix(text: string): string[][] {
  ...
  if (!Array.isArray(parsed)) {
    throw new Error('sv-grid-pro: JSON import expects a top-level array')
  }
