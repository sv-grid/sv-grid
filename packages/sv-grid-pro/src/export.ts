...
async function getDataExporter() {
  if (typeof window === 'undefined') {
    throw new Error('sv-grid-pro: export requires a browser environment')
  }
...
async function ensureGlobals(format: ExportFormat): Promise<void> {
  ...
  if (format === 'xlsx' && g.JSZip == null) {
    ...
    try {
      ...
      throw new Error(
        'sv-grid-pro: xlsx export requires the "jszip" peer dependency. ' +
  ...
  if (format === 'pdf' && g.pdfMake == null) {
    ...
    try {
      ...
      throw new Error(
        'sv-grid-pro: pdf export requires the "pdfmake" peer dependency. ' +
...
>(api: SvGridApi<TFeatures, TData>, opts: ExportOptions<TData>): Promise<void> {
  ...
  if (opts.sheets && opts.sheets.length > 0) {
    if (opts.format !== 'xlsx') {
      throw new Error(`sv-grid-pro: multi-sheet export requires format 'xlsx', got '${opts.format}'`)
    }
