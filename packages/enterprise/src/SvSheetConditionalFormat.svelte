<script lang="ts">
  ...
  const TITLES: Record<CfPreset, string> = {
  ...
  const LEADS: Record<CfPreset, string> = {
    ...
    between: 'Format cells that are BETWEEN:', equal: 'Format cells that are EQUAL TO:',
    text: 'Format cells that contain the text:', duplicates: 'Format cells that contain:',
...
<SvModal bind:open onClose={onClose} title={TITLES[preset]} size="sm" width={440}>
  <form class="sv-sheet-dialog cf" onsubmit={(e) => { e.preventDefault(); ok() }}>
    <div class="where">{address}</div>
    <div class="lead">{LEADS[preset]}</div>
    <div class="row">
      {#if preset === 'between'}
        <input bind:this={first} type="text" bind:value={value1} aria-label="Lower value" spellcheck="false" />
        <span>and</span>
      ...
      {:else if preset === 'top10' || preset === 'bottom10'}
        <input bind:this={first} type="number" min="1" bind:value={rank} aria-label="How many" />
        <label class="check"><input type="checkbox" bind:checked={percent} /> % of selected range</label>
      {/if}
      <span>with</span>
      <select bind:value={styleId} aria-label="Format style">
