...
const STUBS = [
  ...
  { id: 'kanban-from-grid-data',    title: 'Kanban board over the same `$state`', demo: '76-kanban-board',
    when: 'One <SvGrid> with columns = lanes, rows pivoted by status. HTML5 drag-drop moves cards between lanes.',
    api: ["pivot the source array into row × lane slots", "drag/drop events on cells"] },
